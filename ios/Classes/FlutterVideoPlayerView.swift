...
class VideoPlayerView: NSObject, FlutterPlatformView, PlayerViewDelegate {
    ...
    init(
        ...
        if let args = args as? Dictionary<String, Any>,
           let creationId = args["creationId"] as? Int{
            print("creationId: \(creationId)")
            _channel = FlutterMethodChannel(name: "oneplusdream/video_channel_\(creationId)", binaryMessenger: registrar.messenger())
    ...
    func createNativeView(view _view: UIView, arguments args: Any?){
        ...
        guard let args = args as? Dictionary<String, Any>,
            ...
            return}
        if(items.count == 0) {
            showToast(message: "Playing items is not set, please configure one")
            return;
        }
    ...
    func onUrlRequested(url: String) {
        self._channel.invokeMethod("onUrlRequested", arguments: url)
    }
